...
input double         StopLoss                                = 6000.0;     // 止损金额
...
int                  g_profit_cache_day_key = -1;
bool                 g_profit_cache_resync_needed = false;
...
int        ProfitDayKey(const datetime value);
datetime   DayStartTime(const datetime value);
double     ClosedProfitForPeriod(const datetime from_time,const datetime to_time);
double     CurrentFloatingProfitForPanel();
void       InitClosedProfitCache();
void       CheckProfitCacheDayChange();
double     ClosedDealProfit(const ulong deal_ticket);
bool       IsClosedEaDeal(const ulong deal_ticket);
bool       PositionIdBelongsToEaInHistory(const long position_id);
void       CreateSimplePanelObjects();
void       DeletePanelObjects();
void       SetPanelLabel(const string name,const string text,const int x,const int y,const color clr,const int font_size=10);
void       SetPanelButton(const string name,const string text,const int x,const int y,const color text_color);
...
void OnChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam)
  {
   ...
   if(sparam == g_panel_prefix + "trade_button")
     {
      const bool running=(g_allow_buy || g_allow_sell);
      g_allow_buy=!running;
      g_allow_sell=!running;
      ObjectSetInteger(0,sparam,OBJPROP_STATE,false);
      RefreshPanel(true);
      return;
     }

   if(sparam == g_panel_prefix + "close_all_button")
     {
...
datetime DayStartTime(const datetime value)
  {
   return(StringToTime(TimeToString(value,TIME_DATE) + " 00:00"));
  }
...
bool IsClosedEaDeal(const ulong deal_ticket)
  {
   ...
   if(entry != DEAL_ENTRY_OUT && entry != DEAL_ENTRY_OUT_BY && entry != DEAL_ENTRY_INOUT)
    ...
    if((long)HistoryDealGetInteger(deal_ticket,DEAL_MAGIC) != Magic)
      {
       const long position_id=(long)HistoryDealGetInteger(deal_ticket,DEAL_POSITION_ID);
       if(!PositionIdBelongsToEaInHistory(position_id))
          return(false);
      }
    return(true);
  }
...
double ClosedProfitForPeriod(const datetime from_time,const datetime to_time)
  {
   ...
   const int deals_total=(int)HistoryDealsTotal();
   ulong deal_tickets[];
   ArrayResize(deal_tickets,deals_total);
   for(int i=0; i<deals_total; ++i)
      deal_tickets[i]=HistoryDealGetTicket(i);

   for(int i=0; i<deals_total; ++i)
     {
      const ulong deal_ticket=deal_tickets[i];
      if(IsClosedEaDeal(deal_ticket))
         total+=ClosedDealProfit(deal_ticket);
     }
   return(total);
  }
...
void InitClosedProfitCache()
  {
   const datetime now_value=ReferenceNow();
   const datetime today_start=DayStartTime(now_value);
   const datetime yesterday_start=today_start - 86400;
   g_profit_cache_day_key=ProfitDayKey(now_value);
   g_today_closed_profit=ClosedProfitForPeriod(today_start,now_value);
   g_yesterday_closed_profit=ClosedProfitForPeriod(yesterday_start,today_start - 1);
  }

void CheckProfitCacheDayChange()
  {
   const datetime now_value=ReferenceNow();
   const int day_key=ProfitDayKey(now_value);
   if(g_profit_cache_day_key == day_key)
      return;

   const datetime today_start=DayStartTime(now_value);
   g_yesterday_closed_profit=ClosedProfitForPeriod(today_start - 86400,today_start - 1);
   g_today_closed_profit=0.0;
...
void CreateSimplePanelObjects()
  {
   ...
   const color bg=C'10,14,20';
   const color border=C'45,55,70';
   ObjectCreate(0,g_panel_prefix + "panel",OBJ_RECTANGLE_LABEL,0,0,0);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_CORNER,corner);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_XDISTANCE,8);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_YDISTANCE,18);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_XSIZE,260);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_YSIZE,238);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_BGCOLOR,bg);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_COLOR,border);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_BACK,false);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_SELECTABLE,false);
   ObjectSetInteger(0,g_panel_prefix + "panel",OBJPROP_HIDDEN,true);

   const string names[]={"title","symbol","period","status","floating","today","yesterday"};
   for(int i=0; i<ArraySize(names); ++i)
     {
      const string name=g_panel_prefix + names[i];
      ObjectCreate(0,name,OBJ_LABEL,0,0,0);
      ObjectSetInteger(0,name,OBJPROP_CORNER,corner);
      ObjectSetInteger(0,name,OBJPROP_SELECTABLE,false);
      ObjectSetInteger(0,name,OBJPROP_HIDDEN,true);
      ObjectSetString(0,name,OBJPROP_FONT,"Microsoft YaHei UI");
       ObjectSetInteger(0,name,OBJPROP_FONTSIZE,(names[i] == "title") ? 12 : 10);
      }

   const string button_names[]={"trade_button","close_all_button"};
   for(int i=0; i<ArraySize(button_names); ++i)
     {
      const string name=g_panel_prefix + button_names[i];
      ObjectCreate(0,name,OBJ_BUTTON,0,0,0);
      ObjectSetInteger(0,name,OBJPROP_CORNER,corner);
      ObjectSetInteger(0,name,OBJPROP_SELECTABLE,false);
      ObjectSetInteger(0,name,OBJPROP_HIDDEN,true);
      ...
      ObjectSetInteger(0,name,OBJPROP_BGCOLOR,C'28,34,44');
...
void SetPanelLabel(const string name,const string text,const int x,const int y,const color clr,const int font_size=10)
  {
   const string obj=g_panel_prefix + name;
...
void DrawPanel(const EAStats &stats)
  {
   const color white=C'235,240,246';
   const color muted=C'160,168,178';
   const color green=C'80,210,120';
   const color red=C'230,80,80';
   const color gold=C'255,205,80';
   const bool running=(g_allow_buy || g_allow_sell);
   const double floating=CurrentFloatingProfitForPanel();
   const string period_text=TimeframeLabel((ENUM_TIMEFRAMES)Period());
   const datetime now_value=ReferenceNow();
   const datetime today_start=TodayAt("00:00",now_value);
   const double today_profit=TodayClosedProfit(now_value);
   const double yesterday_profit=CalculateClosedProfit(today_start - 86400,today_start - 1,Magic,true);
   const double today_progress=today_profit + floating;

   SetPanelLabel("title","多米诺量化 M3",18,28,white,12);
   SetPanelLabel("symbol","品种：" + _Symbol,18,54,muted,10);
   SetPanelLabel("period","周期：" + period_text,18,76,muted,10);
   SetPanelLabel("status","交易状态：" + (running ? "运行中" : "已停止"),18,102,running ? green : gold,10);
   SetPanelLabel("floating","今日进度：" + DoubleToString(today_progress,2),18,126,today_progress >= 0.0 ? green : red,10);
   SetPanelLabel("today","今日已平：" + DoubleToString(today_profit,2),18,150,today_profit >= 0.0 ? green : red,10);
   SetPanelLabel("yesterday","昨日已平：" + DoubleToString(yesterday_profit,2),18,174,yesterday_profit >= 0.0 ? green : red,10);
   SetPanelButton("trade_button",running ? "停止交易" : "开始交易",18,202,running ? gold : green);
   SetPanelButton("close_all_button","全部平仓",136,202,white);
  }
...
void RefreshPanel(const bool force)
  {
   ...
   EAStats empty_stats;
   ResetStats(empty_stats);
   DrawPanel(empty_stats);
